# 😶‍🌫️ Dijital Alışkanlık mı, Bilinçli Sadakat mi? Yemek Sipariş Uygulamalarında Kuşaklar Arası Ekonomik Algı Farkları

## 📌 Temel Bilgiler ve Proje Amacı

### Slogan
> *"Z Kuşağı otopilot modda mı sipariş veriyor, yoksa ekonomik krizde bilinçli tercihler mi yapıyor?"*

### Projenin Özü
Bu araştırma, yemek sipariş uygulamalarında **üç kritik dinamiği** birlikte inceler:

1. 🎮 **Alışkanlık Paradoksu:** Gençler teknolojiye bağımlı ama sadık değil mi?
2. 💰 **Ekonomik Kriz Gerçeği:** Enflasyon döneminde fiyat hassasiyeti sadakati nasıl etkiliyor?
3. 📊 **Kuşak Farkları:** Hangi kuşak daha sadık, hangisi daha fiyat odaklı?

---

## 🔍 Araştırma Soruları

### Ana Araştırma Sorusu
**"Yemek sipariş uygulamalarına yönelik sadakatin belirleyicileri nelerdir ve bu belirleyiciler alışkanlık ve ekonomik algı tarafından nasıl modere edilir?"**

### Alt Araştırma Soruları

**1. Temel İlişkiler:**
- Performans ve çaba beklentisi, algılanan değerleri (duygusal, fiyat) nasıl etkiler?
- Algılanan değerler, sadakati ne ölçüde tahmin eder?

**2. Kuşak Dinamikleri:**
- Z, Y ve X kuşakları arasında alışkanlık düzeyleri farklılaşıyor mu?
- Hangi kuşak için algılanan değerler daha kritik?

**3. Ekonomik Kriz Bağlamı:**
- Ekonomik durumu kötü algılayan kullanıcılar, fiyat değerine daha mı duyarlı?
- Ekonomik algı, fiyat değerinin sadakat üzerindeki etkisini nasıl modere ediyor?

**4. Karmaşık Etkileşimler:**
- Alışkanlık ve ekonomik algının **ikili etkileşimi** var mı?
- Hangi profildeki kullanıcı en sadık?

---

## 📋 HİPOTEZLER

### A) ANA ETKİLER (Direct Effects)

#### Grup 1: Performans & Çaba Beklentisi → Algılanan Değerler

**H1a:** Performans ve çaba beklentisi, duygusal değeri pozitif etkiler (β > 0, p < 0.05)

**H1b:** Performans ve çaba beklentisi, fiyat değerini pozitif etkiler (β > 0, p < 0.05)

---

#### Grup 2: Algılanan Değerler → Sadakat

**H2:** Duygusal değer ve fiyat değeri, sadakati pozitif etkiler (β > 0, p < 0.05)
- H2a: Duygusal değer → Sadakat (+)
- H2b: Fiyat değeri → Sadakat (+)

**Beklenti:** β(Duygusal) > β(Fiyat)

---

#### Grup 3: Kuşak Farkları

**H3a:** Alışkanlık düzeyi, kuşaklar arasında anlamlı fark gösterir (F istatistiği, p < 0.05)

**H3b:** Z kuşağı > Y kuşağı > X kuşağı (alışkanlık skorunda)

---

### B) MODERASYON ETKİLERİ (Moderation Effects)

#### Moderatör 1: Alışkanlık

**H4 (Temel Moderasyon):** Alışkanlık, "algılanan değerler → sadakat" ilişkisini modere eder

**H4a:** Düşük alışkanlık grubunda, duygusal değerin sadakat üzerindeki etkisi daha güçlüdür (β_düşük > β_yüksek, p < 0.05)

**H4b:** Düşük alışkanlık grubunda, fiyat değerinin sadakat üzerindeki etkisi daha güçlüdür (β_düşük > β_yüksek, p < 0.05)

---

#### Moderatör 2: Ekonomik Kriz Algısı

**H5:** Ekonomik durum algısı, "fiyat değeri → sadakat" ilişkisini modere eder

**H5a:** Ekonomik durumu kötü algılayanlarda, fiyat değerinin sadakat üzerindeki etkisi daha güçlüdür (β_kötü > β_iyi, p < 0.05)

**H5b:** Ekonomik durumu iyi algılayanlarda, duygusal değerin sadakat üzerindeki etkisi daha güçlüdür (β_iyi > β_kötü, p < 0.05)

---

## 📊 7'Lİ LİKERT ÖLÇEKLERİ VE HİPOTEZ BAĞLANTILARI

### Ölçek Formatı

1 = Kesinlikle Katılmıyorum
2 = Katılmıyorum
3 = Kısmen Katılmıyorum
4 = Ne Katılıyorum Ne Katılmıyorum
5 = Kısmen Katılıyorum
6 = Katılıyorum
7 = Kesinlikle Katılıyorum

---

### BLOK 1: Performans & Çaba Beklentisi

**Talimat:** *"En sık kullandığınız yemek sipariş uygulamasını düşünerek aşağıdaki ifadelere katılım düzeyinizi belirtiniz."*

| Kod | Madde | Bağlı Hipotez |
|-----|-------|---------------|
| **PE1** | Bu uygulama günlük yaşamımda kullanışlıdır | H1a, H1b |
| **PE2** | Bu uygulama satın alma sürecimi hızlandırır | H1a, H1b |
| **EE1** | Bu uygulamayı kullanmak kolaydır | H1a, H1b |
| **EE2** | Bu uygulamayla etkileşimim net ve anlaşılırdır | H1a, H1b |

**Kompozit skor:** PERF_EFFORT = (PE1 + PE2 + EE1 + EE2) / 4

---

### BLOK 2: Algılanan Değerler

#### A) Duygusal Değer

| Kod | Madde | Bağlı Hipotez |
|-----|-------|---------------|
| **EV1** | Bu uygulamayı kullanırken rahat hissediyorum | H2a, H4a, H5b |
| **EV2** | Bu uygulamayı kullanırken keyif alıyorum | H2a, H4a, H5b |

**Kompozit skor:** EMOTIONAL_VALUE = (EV1 + EV2) / 2

---

#### B) Fiyat Değeri

| Kod | Madde | Bağlı Hipotez |
|-----|-------|---------------|
| **PV1** | Bu uygulama farklı restoranların fiyatlarını karşılaştırarak tasarruf etmemi sağlar | H2b, H4b, H5a |
| **PV2** | Bu uygulamanın fiyatları makuldür | H2b, H4b, H5a |

**Kompozit skor:** PRICE_VALUE = (PV1 + PV2) / 2

---

### BLOK 3: Sadakat (Bağımlı Değişken)

| Kod | Madde | Bağlı Hipotez |
|-----|-------|---------------|
| **LOY1** | Bu uygulama benim ilk tercihim olacaktır | H2, H4, H5 |
| **LOY2** | Bu uygulamayı başkalarına tavsiye ederim | H2, H4, H5 |
| **LOY3** | Bu uygulamadan vereceğim sipariş sayısı artacak | H2, H4, H5 |

**Kompozit skor:** LOYALTY = (LOY1 + LOY2 + LOY3) / 3

---

### BLOK 4: Alışkanlık (Moderatör 1)

| Kod | Madde | Bağlı Hipotez |
|-----|-------|---------------|
| **HAB1** | Bu uygulamadan yemek sipariş etmek benim için neredeyse bir alışkanlık | H3a, H3b, H4a, H4b |
| **HAB2** | Yemek siparişi için bu uygulamayı kullanmaya bağımlıyım | H3a, H3b, H4a, H4b |

**Kompozit skor:** HABIT = (HAB1 + HAB2) / 2

**Moderasyon grupları:**
- Düşük alışkanlık: HABIT < 4.0
- Yüksek alışkanlık: HABIT ≥ 4.0

---

## 👁️ DİKKAT KONTROLLERİ

### Teknik 1: Instructed Response Items (IRI)

**Soru (Ana ölçekler arasına gizlenmiş):**
> *"Bu soruyu dikkatlice okuduğunuzu göstermek için lütfen '2 - Katılmıyorum' seçeneğini işaretleyiniz."*

**Çıkarma kriteri:** Farklı cevap veren → Anketten çıkar

---

### Teknik 2: Zamanlama Kontrolü

**Kritik eşik:** 
- Çok hızlı: < 3 dakika → Çıkar
- Çok yavaş: > 20 dakika → Çıkar

---

### Teknik 3: Straight-Lining Kontrolü

**Post-hoc:** Tüm Likert sorularına aynı cevabı verenler → Çıkar

---

## 🎯 KONTROL DEĞİŞKENLERİ

### 1. Kullanım Sıklığı
**Soru:** Aylık ortalama kaç kez yemek sipariş verirsiniz?
- [ ] 0-2 kez
- [ ] 3-5 kez
- [ ] 6-10 kez
- [ ] 10+ kez

---

### 2. Platform Çeşitliliği
**Soru:** Kaç farklı yemek sipariş uygulaması kullanıyorsunuz?
- [ ] Sadece 1 uygulama
- [ ] 2 uygulama
- [ ] 3-4 uygulama
- [ ] 5+ uygulama

---

### 3. Kampanya Duyarlılığı (7'li Likert)
**Soru:** "Sadece kampanya/indirim varsa sipariş veririm"
- 1 (Kesinlikle Katılmıyorum) → 7 (Kesinlikle Katılıyorum)

---

## 📊 DEMOGRAFİK BİLGİLER

### 1. Yaş (Kuşak Analizi İçin)
**Doğum yılınız:** _____ (Açık uçlu)

**Kodlama:**
- Z Kuşağı: 1997-2012 (12-27 yaş)
- Y Kuşağı: 1981-1996 (28-43 yaş)
- X Kuşağı: 1965-1980 (44-59 yaş)

---

### 2. Cinsiyet
- [ ] Kadın
- [ ] Erkek
- [ ] Diğer
- [ ] Belirtmek istemiyorum

---

### 3. Eğitim Durumu
- [ ] Lise veya altı
- [ ] Ön lisans
- [ ] Lisans
- [ ] Yüksek lisans
- [ ] Doktora

---

### 4. Meslek
- [ ] Öğrenci
- [ ] Tam zamanlı çalışan
- [ ] Yarı zamanlı çalışan
- [ ] Serbest meslek
- [ ] Emekli
- [ ] İşsiz

---

### 5. Ekonomik Durum Algısı (H5 için KRİTİK!)

**Soru:** Ekonomik durumunuz hakkında nasıl hissediyorsunuz?
- [ ] Çok kötü (Gelirim giderlerimi karşılamıyor)
- [ ] Kötü (Zorlukla karşılıyorum)
- [ ] Orta (İdare ediyorum)
- [ ] İyi (Rahat yaşıyorum)
- [ ] Çok iyi (Tasarruf yapabiliyorum)

**Analiz için gruplandırma:**
- Kötü ekonomik durum: 1-2 puan
- İyi ekonomik durum: 4-5 puan

---

### 6. Tercih Edilen Uygulama
**En sık kullandığınız uygulama:**
- [ ] Yemeksepeti
- [ ] Getir Yemek
- [ ] Trendyol Yemek
- [ ] Diğer: ___

---

### 7. Şehir
**Yaşadığınız şehir:** _____

---

## 📝 ANKET AKIŞI

### Anket Yapısı

| Bölüm | Madde Sayısı | Tahmini Süre |
|-------|--------------|--------------|
| **GİRİŞ** | | |
| Filtre Sorusu | 1 soru | 0.2 dk |
| **ANA ÖLÇEKLER** | | |
| Performans & Çaba Beklentisi | 4 madde | 0.8 dk |
| Duygusal Değer | 2 madde | 0.4 dk |
| Fiyat Değeri | 2 madde | 0.4 dk |
| Sadakat | 3 madde | 0.6 dk |
| Alışkanlık | 2 madde | 0.4 dk |
| Dikkat Kontrolü (gizli) | 1 madde | 0.3 dk |
| **KONTROL DEĞİŞKENLERİ** | | |
| Kullanım Sıklığı, Platform, Kampanya | 3 madde | 0.5 dk |
| **DEMOGRAFİK** | | |
| Yaş, Cinsiyet, Eğitim, Meslek, Ekonomik Durum, Uygulama, Şehir | 7 soru | 1.0 dk |
| **TOPLAM** | **25 soru** | **~4.5 dakika** |

---

## 🎯 KOMPLE ANKET FORMU

---

### **BÖLÜM 0: FİLTRE SORUSU**

**F1. Son 6 ay içinde yemek sipariş uygulaması kullandınız mı?**
- [ ] Evet → Ankete devam edin
- [ ] Hayır → Anket sonlandırılır

> *"Teşekkür ederiz. Bu anket yemek sipariş uygulaması kullanıcılarına yöneliktir."*

---

### **GİRİŞ METNİ**

> **Sayın Katılımcı,**
>
> Bu anket, yemek sipariş uygulamalarının kullanımı ile ilgili akademik bir araştırma çalışmasıdır. Lütfen **en sık kullandığınız** yemek sipariş uygulamasını düşünerek soruları cevaplayınız.
>
> Anket tamamen **gizlidir** ve verileriniz yalnızca bilimsel amaçlarla kullanılacaktır. Anketin tamamlanması yaklaşık **4-5 dakika** sürecektir.
>
> Katılımınız için teşekkür ederiz.

---

### **BÖLÜM 1: PERFORMANS & ÇABA BEKLENTİSİ**

**Talimat:** *"En sık kullandığınız yemek sipariş uygulamasını düşünerek aşağıdaki ifadelere ne ölçüde katıldığınızı belirtiniz."*

| Kod | İfade | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|-----|-------|---|---|---|---|---|---|---|
| **PE1** | Bu uygulama günlük yaşamımda kullanışlıdır | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **PE2** | Bu uygulama satın alma sürecimi hızlandırır | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **EE1** | Bu uygulamayı kullanmak kolaydır | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **EE2** | Bu uygulamayla etkileşimim net ve anlaşılırdır | ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **BÖLÜM 2: DUYGUSAL DEĞER**

**Talimat:** *"En sık kullandığınız yemek sipariş uygulaması için aşağıdaki ifadelere ne ölçüde katıldığınızı belirtiniz."*

| Kod | İfade | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|-----|-------|---|---|---|---|---|---|---|
| **EV1** | Bu uygulamayı kullanırken rahat hissediyorum | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **EV2** | Bu uygulamayı kullanırken keyif alıyorum | ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **BÖLÜM 3: FİYAT DEĞERİ**

**Talimat:** *"En sık kullandığınız yemek sipariş uygulaması için aşağıdaki ifadelere ne ölçüde katıldığınızı belirtiniz."*

| Kod | İfade | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|-----|-------|---|---|---|---|---|---|---|
| **PV1** | Bu uygulama farklı restoranların fiyatlarını karşılaştırarak tasarruf etmemi sağlar | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **PV2** | Bu uygulamanın fiyatları makuldür | ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **DİKKAT KONTROLÜ (Gizli - Rastgele Yerleştirilmiş)**

| İfade | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|-------|---|---|---|---|---|---|---|
| **ATT1** | Bu soruyu dikkatlice okuduğunuzu göstermek için lütfen "2 - Katılmıyorum" seçeneğini işaretleyiniz | ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **BÖLÜM 4: SADAKAT**

**Talimat:** *"En sık kullandığınız yemek sipariş uygulaması için gelecekteki niyetlerinizi belirtiniz."*

| Kod | İfade | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|-----|-------|---|---|---|---|---|---|---|
| **LOY1** | Bu uygulama benim ilk tercihim olacaktır | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **LOY2** | Bu uygulamayı başkalarına tavsiye ederim | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **LOY3** | Bu uygulamadan vereceğim sipariş sayısı artacak | ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **BÖLÜM 5: ALIŞKANLIK**

**Talimat:** *"En sık kullandığınız yemek sipariş uygulaması ile ilgili kullanım alışkanlıklarınızı değerlendiriniz."*

| Kod | İfade | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|-----|-------|---|---|---|---|---|---|---|
| **HAB1** | Bu uygulamadan yemek sipariş etmek benim için neredeyse bir alışkanlık | ○ | ○ | ○ | ○ | ○ | ○ | ○ |
| **HAB2** | Yemek siparişi için bu uygulamayı kullanmaya bağımlıyım | ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **BÖLÜM 6: KONTROL DEĞİŞKENLERİ**

**K1. Aylık ortalama kaç kez yemek sipariş verirsiniz?**
- [ ] 0-2 kez
- [ ] 3-5 kez
- [ ] 6-10 kez
- [ ] 10+ kez

---

**K2. Kaç farklı yemek sipariş uygulaması kullanıyorsunuz?**
- [ ] Sadece 1 uygulama
- [ ] 2 uygulama
- [ ] 3-4 uygulama
- [ ] 5+ uygulama

---

**K3. Aşağıdaki ifadeye ne ölçüde katılıyorsunuz:**

*"Sadece kampanya/indirim varsa sipariş veririm"*

| 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|---|---|---|---|---|---|---|
| ○ | ○ | ○ | ○ | ○ | ○ | ○ |

---

### **BÖLÜM 7: DEMOGRAFİK BİLGİLER**

**D1. Doğum yılınız:** _____ (Açık uçlu)

---

**D2. Cinsiyetiniz:**
- [ ] Kadın
- [ ] Erkek
- [ ] Diğer
- [ ] Belirtmek istemiyorum

---

**D3. Eğitim durumunuz:**
- [ ] Lise veya altı
- [ ] Ön lisans
- [ ] Lisans
- [ ] Yüksek lisans
- [ ] Doktora

---

**D4. Mesleğiniz:**
- [ ] Öğrenci
- [ ] Tam zamanlı çalışan
- [ ] Yarı zamanlı çalışan
- [ ] Serbest meslek
- [ ] Emekli
- [ ] İşsiz

---

**D5. Ekonomik durumunuz hakkında nasıl hissediyorsunuz?**
- [ ] Çok kötü (Gelirim giderlerimi karşılamıyor)
- [ ] Kötü (Zorlukla karşılıyorum)
- [ ] Orta (İdare ediyorum)
- [ ] İyi (Rahat yaşıyorum)
- [ ] Çok iyi (Tasarruf yapabiliyorum)

---

**D6. En sık kullandığınız yemek sipariş uygulaması:**
- [ ] Yemeksepeti
- [ ] Getir Yemek
- [ ] Trendyol Yemek
- [ ] Diğer: _____

---

**D7. Yaşadığınız şehir:** _____ (Açık uçlu)

---

### **BİTİŞ METNİ**

> **Anketimize katıldığınız için çok teşekkür ederiz!**
>
> Verdiğiniz cevaplar bilimsel araştırmamıza önemli katkı sağlayacaktır.
>
> İyi günler dileriz.

---

## 📊 HİPOTEZ ÖZETİ

| Hipotez Türü | Sayı | Test Yöntemi |
|--------------|------|--------------|
| Ana Etkiler | 4 (H1a-b, H2a-b) | Doğrusal/Çoklu Regresyon |
| Kuşak Farkı | 2 (H3a-b) | ANOVA + Post-hoc Tukey |
| Moderasyon (Alışkanlık) | 2 (H4a-b) | Çoklu Grup Regresyonu / Etkileşim Terimi |
| Moderasyon (Ekonomik) | 2 (H5a-b) | Çoklu Grup Regresyonu / Etkileşim Terimi |
| **TOPLAM** | **10 hipotez** | - |

---

## 🎯 ÖRNEKLEM HEDEFİ

**Toplam:** 250-300 geçerli yanıt

**Kuşak dağılımı:**
- Z Kuşağı: ~80-100 kişi (33%)
- Y Kuşağı: ~100-120 kişi (40%)
- X Kuşağı: ~70-80 kişi (27%)

---

## 📋 DEĞİŞKEN LİSTESİ

**Bağımsız Değişkenler:**
- Performans & Çaba Beklentisi (PERF_EFFORT)

**Aracı Değişkenler:**
- Duygusal Değer (EMOTIONAL_VALUE)
- Fiyat Değeri (PRICE_VALUE)

**Bağımlı Değişken:**
- Sadakat (LOYALTY)

**Moderatör Değişkenler:**
- Alışkanlık (HABIT)
- Ekonomik Durum Algısı (ECONOMIC_PERCEPTION)

**Kontrol Değişkenleri:**
- Kullanım Sıklığı
- Platform Çeşitliliği
- Kampanya Duyarlılığı
- Kuşak (AGE_GROUP)
- Cinsiyet, Eğitim, Meslek

---

## 🔬 ANALİZ PLANI

### Adım 1: Veri Temizleme
- Dikkat kontrolü başarısız olanlar → Çıkar
- Süre kontrolü (< 3 dk veya > 20 dk) → Çıkar
- Straight-lining yapanlar → Çıkar
- Hedef: En az 250 geçerli veri

### Adım 2: Tanımlayıcı İstatistikler
- Demografik frekanslar
- Kuşak dağılımları
- Ortalama, standart sapma, çarpıklık, basıklık

### Adım 3: Güvenilirlik Analizi
- Cronbach's Alpha (her ölçek için)
- Hedef: α > 0.70

### Adım 4: Ana Etkiler Testi
- H1a-b, H2a-b: Çoklu regresyon analizi
- H3a-b: One-Way ANOVA + Tukey post-hoc

### Adım 5: Moderasyon Analizleri
- H4a-b: Alışkanlık moderasyonu (Çoklu grup regresyon)
- H5a-b: Ekonomik algı moderasyonu (Çoklu grup regresyon)

### Adım 6: Model Uyumu (İsteğe Bağlı)
- Path analysis veya SEM
- Fit indeksleri: CFI, TLI, RMSEA

---

## 📚 BEKLENEN KATKILAR

### Teorik:
1. UTAUT2 + VAB + Ekonomik Kriz Teorisi (ilk kez birlikte)
2. İki moderatörün karşılaştırmalı analizi
3. Kuşaklar arası farklılıkların çok boyutlu incelenmesi

### Pratik:
1. Kuşak-özel pazarlama stratejileri
2. Ekonomik durum-özel kampanya tasarımı
3. Alışkanlık segmentasyonu

---
